...
const STOPWORDS: &[&str] = &[
    ...
    "any", "all", "every", "each", "other", "both", "few", "many", "most",
    "much", "such", "same", "own", "than", "next", "first", "last", "long",
    "new", "old", "big", "small", "little", "high", "low", "lot", "lots",
    "always", "never", "sometimes", "often", "usually", "almost",
];
...
mod tests {
    ...
    fn detects_repeated_phrase() {
        ...
        assert!(out.iter().any(|j| j.phrase == "secret garden" && j.count >= 3));
    }
    ...
    fn skips_phrases_with_stopwords() {
        ...
        assert!(out.is_empty(), "expected stopwords to suppress, got {:?}", out);
    }
    ...
    fn counts_trigrams() {
        ...
        let out = detect_inside_jokes(&messages, &InsideJokesConfig::default());
        assert!(out.iter().any(|j| j.phrase == "famous purple rocket" && j.count >= 3));
    }
}
